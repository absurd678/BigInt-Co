...
// ==================== УПРОЩЕННАЯ ВЕРСИЯ AKS ====================

bool DeterministicPrimality::aksTestSimple(const BigInt& n) {
    // Упрощенная версия для сравнения производительности
    if (n < BigInt(2)) return false;
    if (n == BigInt(2)) return true;
    if (n.isEven()) return false;
    
    // Простая проверка малых делителей
    for (BigInt a(2); a < BigInt(1000) && a < n; a = a + BigInt(1)) {
        if (n % a == BigInt(0)) {
            return false;
        }
    }
    
    // Упрощенная проверка полиномиального условия
    for (BigInt a(1); a < BigInt(10); a = a + BigInt(1)) {
        // Проверяем (x + a)^n ≡ x^n + a (mod n) для нескольких x
        for (BigInt x(2); x < BigInt(10); x = x + BigInt(1)) {
            BigInt left = BigInt::modPow(x + a, n, n);
            BigInt right = (BigInt::modPow(x, n, n) + a) % n;
            if (left != right) {
                return false;
...
void DeterministicPrimality::compareTests(const BigInt& n) {
    ...
    auto aks_simple_pair = measureTestSpeed(aksTestSimple, n, "AKS тест (упрощенный)");
    bool aks_simple_result = aks_simple_pair.first;
    double aks_simple_time = aks_simple_pair.second;
    ...
    if (aks_result == miller_result && aks_simple_result == miller_result) {
    ...
    if (aks_time > 0 && miller_time > 0) {
        ...
        cout << "Тест Миллера быстрее упрощенного AKS в " << fixed << setprecision(0) << (aks_simple_time / miller_time) << " раз" << endl;
    }
...
void DeterministicPrimality::runComprehensiveBenchmark() {
    ...
    cout << "\nAKS ТЕСТ (упрощенный):" << endl;
    cout << "  - Быстрая аппроксимация" << endl;
    cout << "  - Подходит для небольших чисел" << endl;
    cout << "  - Не является настоящим AKS" << endl;
