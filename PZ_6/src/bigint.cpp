...
int BigInt::toInt() const {
    ...
    // Проверим, не слишком ли много цифр
    if (digits.size() > 10) { // Максимум 10 цифр у 2147483647
        throw std::overflow_error("BigInt too large to fit in int");
    }

    // Преобразуем в long long для проверки
    long long result = 0;
    long long power = 1;
    for (size_t i = 0; i < digits.size(); ++i) {
    ...
    if (result < MIN_INT || result > MAX_INT) {
        throw std::overflow_error("BigInt value out of range for int");
    }
...
BigInt BigInt::log(const BigInt& n) {
    ...
    // Простое приближение: log(n) ≈ количество_цифр * 2.3
    // Так как log10(n) ≈ количество цифр, а ln(n) = log10(n) * ln(10) ≈ количество цифр * 2.302585
    int digit_count = n.toString().length();
    return BigInt(digit_count * 2); // Упрощенное приближение
}


// Вспомогательная: 10^n как BigInt (без переполнения встроенных типов).
static BigInt pow10_big(size_t n) {
    BigInt r(1);
    BigInt ten(10);
    for (size_t i = 0; i < n; ++i) r = r * ten;
    return r;
}

//---------------------- ПЗ 6 ----------------------------------

// -------------------------  КОНГРУЕНТНЫЙ ГЕНЕРАТОР -------------------------------
BigInt& BigInt::initLCG(size_t numDigits,
                        const BigInt& seed,
                        const BigInt& a,
                        const BigInt& c)
{
    if (numDigits == 0) {
        // 10^0 = 1, модуль 1 даёт 0 — по смыслу вернём 0.
        *this = BigInt(0);
        return *this;
    }

    // модуль m = 10^{numDigits}
    BigInt m = BigInt(10)^BigInt(numDigits);

    // x = (a * seed + c) mod m
    BigInt x = (a * seed + c) % m;

    // Гарантируем ровно numDigits цифр (без ведущего нуля в старшем разряде):
    // если разрядов меньше, поднимем в диапазон [10^{d-1}, 10^{d}) добавив 10^{d-1}.
    size_t d = x.getDigitCount();
    if (x.isZero() || d < numDigits) {
        BigInt add = pow10_big(numDigits - 1); // 10^{d-1}
        x = x + add; // всё ещё < 10^d, т.к. x < 10^{d-1}
    }
...
BigInt& BigInt::initLCG(size_t numDigits)
{
    // простой источник сида — текущее время в наносекундах
    auto ns = std::chrono::high_resolution_clock::now().time_since_epoch().count();
    BigInt seed(ns);
    return initLCG(numDigits, seed);
}


// ------------------- Вихирь Мерсенна ----------------------------

BigInt& BigInt::initMT(size_t numDigits, uint32_t seed32) {
    MT19937lab mt(seed32);
    fill_random_decimal_digits(numDigits, mt);
    return *this;
}

BigInt& BigInt::initMT(size_t numDigits) {
    // простой сид из времени (наносекунды)
    uint64_t t = static_cast<uint64_t>(
        std::chrono::high_resolution_clock::now().time_since_epoch().count()
    );
    // стянем к 32 битам (смешаем старшую и младшую половины)
    uint32_t seed32 = static_cast<uint32_t>(t ^ (t >> 32));
    MT19937lab mt(seed32);
    fill_random_decimal_digits(numDigits, mt);
    return *this;
}

// Вспомогательная: подчистка до ровно numDigits цифр (старшая != 0)
void BigInt::fill_random_decimal_digits(size_t numDigits, MT19937lab& mt) {
    if (numDigits == 0) {
        return;
    }
    // в BigInt младший разряд — digits[0]
    this->isNegative = false;
    this->digits.assign(numDigits, 0);

    // заполнение младших разрядов: 0..9
    for (size_t i = 0; i + 1 < numDigits; ++i)
        this->digits[i] = mt.uniform_int(0, 9);

    // старший разряд: 1..9
    this->digits[numDigits - 1] = mt.uniform_int(1, 9);

    this->removeLeadingZeros(); // на случай несовпадения инвариантов
}

// ---------------------------- ЭНТРОПИЯ -----------------------------------

BigInt& BigInt::initFromMouseEntropy(size_t numDigits, int duration_ms, int poll_hz)
{
    // 1) собираем координаты курсора (X11)
    auto samples = CursorSamplerX11::collect(duration_ms, poll_hz);

    // 2) «выбеливаем» в 64-битный пул и режем до seed32
    uint64_t pool = entropy_pool_from_samples(samples);
    uint32_t seed32 = (uint32_t)(pool ^ (pool >> 32));

    // 3) инициализируем собственный MT и заполняем десятичные цифры
    MT19937lab mt(seed32);
    fill_random_decimal_digits(numDigits, mt);
    return *this;
}

BigInt& BigInt::initFromMouseEntropy(size_t numDigits,
                                     const std::vector<std::pair<int,int>>& coords)
{
    using clock = std::chrono::high_resolution_clock;
    std::vector<CursorSamplerX11::Sample> s;
    s.reserve(coords.size());
    for(size_t i=0;i<coords.size();++i){
        uint64_t tns = (uint64_t)clock::now().time_since_epoch().count() + (uint64_t)i*17ull;
        s.push_back({coords[i].first, coords[i].second, tns});
    }
    uint64_t pool = entropy_pool_from_samples(s);
    uint32_t seed32 = (uint32_t)(pool ^ (pool >> 32));
    MT19937lab mt(seed32);
    fill_random_decimal_digits(numDigits, mt);
    return *this;
